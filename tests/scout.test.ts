...
// These tests pin the single-flight + read-through cache behaviour of scoutProfile.
...
vi.mock("@/lib/dating/engine", () => ({ buildProfile: (s: unknown) => s }));
...
describe("scoutProfile single-flight", () => {
  it("collapses concurrent misses for the same login into one fetch", async () => {
    ...
    const calls = Promise.all([scoutProfile("Torvalds"), scoutProfile("torvalds"), scoutProfile("TORVALDS")]);
    await flush();
  ...
  it("fetches separately for different logins", async () => {
    ...
    await Promise.all([scoutProfile("alice"), scoutProfile("bob")]);
  ...
  it("does not memoise failures — the next scout retries", async () => {
    ...
    await expect(scoutProfile("torvalds")).rejects.toMatchObject({ type: "ratelimit" });
    expect(fetchProfile).toHaveBeenCalledTimes(1);

    // in-flight entry cleared and nothing cached -> a retry hits GitHub again.
    fetchProfile.mockResolvedValueOnce(payload("torvalds"));
    await expect(scoutProfile("torvalds")).resolves.toMatchObject({ login: "torvalds" });
    expect(fetchProfile).toHaveBeenCalledTimes(2);
  });

  it("serves a cached profile without refetching", async () => {
    fetchProfile.mockResolvedValueOnce(payload("torvalds"));
    await scoutProfile("torvalds"); // populates the cache
    await scoutProfile("torvalds"); // served from cache
    await scoutProfile("torvalds");
    expect(fetchProfile).toHaveBeenCalledTimes(1);
  });
