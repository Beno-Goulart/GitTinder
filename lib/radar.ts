import { TRAIT_LABELS, TRAITS } from "./dating/constants";
import type { Traits } from "./dating/types";
...
export function radarGeometry(stats: Traits, size: number): RadarGeometry {
  const center = size / 2;
  ...
  const outline = (r: number) =>
    TRAITS.map((_, i) => at(r, i))
      .map((v) => `${v.x},${v.y}`)
      .join(" ");

  const vertices = TRAITS.map((key, i) =>
  ...
  return {
    ...
    sectors: TRAITS.map((_, i) => radarSector(center, radius, i)),
    labels: TRAITS.map((key, i) => ({
